...
split_item_test = cell_object.value.split("https://www.trustedchoice.com/l/")
...
for row in sheet_object.iter_rows(min_row=2, max_row=sheet_object.max_row):
    k = row[3].value.split('https://www.trustedchoice.com/l/')[1].split("/")[0].strip()
    r = row[1].value #future state - If there's more than 10 results per state, only print the top 10 results. Determined by rank numbers in the file...
    v = row[0].value
    if k in state_dict:
        state_dict[k] += [v]
    else:
